...
def read_train():
	...
	for i in range(12):
		for j in range(475):
			if i == 0 and j == 0:
				continue
			A = B.T[480*i+j : 480*i+j+5].reshape(-1, )
			X = np.concatenate((X, [A]), axis=0)					#X.shape = (5652, 324)
	return (X, Y)
	

def read_test():
	df_test = pd.read_csv('../data/test.csv', encoding="Big5", header=None).replace('NR', '0').convert_objects(convert_numeric=True)
	test_x = df_test.iloc[:, 6:].values.reshape((240, -1))
...
def Regression(X, Y):
	w = np.array([[0.0] for i in range(180)])
	Delta = np.zeros([180, 1])
	for i in range(10):
		Delta[18*i+9, 0] = 1
	#	Delta[18*i, 0] = 1
	#	Delta[18*i+8, 0] = 1
	...
	for it in range(iteration):
		f = X.dot(w*Delta)
		w_grad = -2*(X.T.dot(Y - f) + Lamda*Delta*w)
		b_grad = -2*np.sum(Y-f)
...
if __name__ == '__main__':
	...
	#model_df.to_csv('parameters.csv', index=False)
